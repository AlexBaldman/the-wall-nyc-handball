...
function rumble(duration = 70, strong = 0.35, weak = 0.55) {
  ...
  const gamepad = navigator.getGamepads?.()[state.input.gamepadIndex];
...
function updateGamepad() {
  ...
  let gamepad = state.input.gamepadIndex === null ? null : gamepads[state.input.gamepadIndex];
  if (!gamepad) {
...
function drawBackground() {
  ...
  const serviceMarkerLength = (COURT.right - COURT.left) * (0.5 / 20);
  ctx.strokeStyle = 'rgba(255, 239, 204, 0.9)';
  ctx.lineWidth = 2;
  ctx.beginPath();
